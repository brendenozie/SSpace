        ...
        {pageList &&
          pageList.map(({ icon, label, slug }, i) => (
            <li key={i} className="last:border-none">
              <NavLink  to={slug} className={({ isActive }) => `flex relative items-center px-3 py-3 font-medium ${ isActive ? "text-glitch-orange" : "text-gray-400" }` } >
                <span className="material-icons-outlined mr-7">{icon}</span>{" "}   {label}
              </NavLink>
            </li>
          ))}
      </ul>
